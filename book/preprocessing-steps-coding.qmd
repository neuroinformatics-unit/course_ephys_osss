...
We will preprocess a new dataset recorded on a 64-channel Cambridge Neurotech probe. Note that we will not apply the `phase_shift` step that was used in [introduction preprocessing section](pipeline-walkthrough.qmd#preprocessing), as that is only relevant for Neuropixel probes.
...
TODO: include some information on the experiment, probably do this on a central 'dataset' page

First we need to download the dataset we will. This dataset is (TODO: brief introduction). For more information on the dataset and how to download any one of the thousands of open-access datasets available on the DANDI archive, please see TODO.
...
def plot_prepro(recordings, time_range):
    """
    ...
    # If only 1 plot is passed, axes will not be 1d
    # (they will be a single Axes object)
...
recording_bp = si_prepro.bandpass_filter(
    recording, 
    # commonly used values used based on AP waveform kinetics
    freq_min=300, 
    freq_max=6000
) 
...
recording_cmr = si_prepro.common_reference(
    rec_clean, 
    operator="median"
)
...
recording_hsf = si_prepro.highpass_spatial_filter(
    rec_clean, 
...
as use different channels as reference using the `ref_channel_ids` argument. Recordings have a `get_channel_ids()` function hat
...
[excellent infographic](https://spikeinterface.readthedocs.io/en/stable/_images/motion_correction_overview.png) for details). Therefore most motion correction algorithms have three main steps:
...
SpikeInterface exposes these through the [`correct_motion()`](https://spikeinterface.readthedocs.io/en/latest/api.html#spikeinterface.preprocessing.correct_motion) function that using convenient presets which define how spike detection, motion estimation and interpolation is run.
...
If the drift plots in your recordings indicate little drift, you may want to consider turning off motion correction. 
This is because the interpolation step will be performed based on small, but non-zero estimated motion and can add noise to the recording. 

Further, motion correction 
...
rate neurons (see [here](https://github.com/SpikeInterface/spikeinterface/issues/4432) for a discussion on the phenomenon) e.g:
...
The takeaway from this section is to always plot your estimation motion (even if you don't eventually apply motion correction), using it to make the decision whether to apply this to your sorted data or not.
...
We can estimate the original channel scaling by multiplying the data by the average standard deviation (i.e. noise) of the data. This scales back up the variances along the covariance matrix to approximate that in the original dataset.

:::

```{python}

noise = np.mean(si.get_noise_levels(
