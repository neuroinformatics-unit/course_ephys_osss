...
First we need to download the dataset we will use in this chapter, and through later sections of the course.
This dataset is from a 64-channel Cambridge Neurotech recording in the postsubiculum. See the [course data page](course-data.qmd#head-direction-experiment-from-the-dandi-archive) for more information on the dataset, details on how it is downloaded, and how you can similarly download any one of the thousands of open-access datasets available on the DANDI archive.

For now, we will use a convenience function to download the recording of interest:

```{python}

LOCAL_FOLDER = Path("_tmp/dandiset-000939-90s")
