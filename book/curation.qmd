---
title: "Visualization and manual curation"
execute:
  eval: false
---
...
On Monday and Tuesday we saw how to take raw ephys data, preprocess it, sort it and compute properties of that sorting. When you're making a spike sorting pipeline you have lots of things to check and decisions to make. But once you've got the pipeline fixed, the final product is often quite simple. Here's an example of a full basic pipeline for a chronic recording:

```{python}
import spikeinterface.full as si

from utils import download_and_slice_nwb_from_dandi
dataset_folder = Path("data/nwb_recording")

raw_recording = si.read_nwb()
...
If you have annoying things like motion, artefacts, seperate shanks, a custom probe, multiple recordings, etc you'll need to modify this simple pipeline. But this is a good starting point! The end goal of a SpikeInterface pipeline is the Sorting Analyzer - and we've just made one!

While we were preprocessing and spike sorting on Monday and Tuesday, we used a 90 second snippet of a recording from Adrian Duszkiewicz available at [this DANDIlink](https://dandiarchive.org/dandiset/000939/0.260512.1701/files?location=sub-A3702). You can keep using this if you'd like. However, it will be more fun (and realistic) to use a longer recording. I've made an analyzer computed using the same recording as we used earlier in the week, but sorted using the full 2 hour duration of the recording. You can download it HERE, or it's also on a usb stick in the room! Whichever you decide to use, please make a note of where your analyzer is stored. We'll say that the analyzer is at `path/to/analyzer`. Change this when you see it.
...
It's helpful to remind ourselves what the definition of "neuron" and "unit" are:

  - **Neuron**: a neuron, an actual cell.
  - **Unit**: something a spike sorter thinks is a neuron

Note that some people call units "clusters". The common "mistakes" that spike sorters make are:

  1) Include a unit which is clearly noise or an artefact
...
In SpikeInterface a **Curation** is a list of decisions. These decisions are labels, merges and splits. As we proceed, we'll create **Curation**s by using the GUI or by using tools inside SpikeInterface.
...
Suppose you are interested in very specific cells. Ones that only respond to certain stimuli and live in a specific brain area. You're expecting to get a handful of these per recording. In this case, your scientific question depends sensitively on the fact you're looking at a good unit. Hence you need to go in a surgically check that each unit you keep is "good".

On the other hand, maybe you're doing population level questions. How does the overall firing rate different between brain states? What are the low-dimensional dynamics? In this case, the individual nature of each unit is less important (or can be important, depending on your methods...). Hence you can be a bit less careful: get rid of the stuff that's obviously noise and continue.
...
"Curation mode" allows us to label units (and merge and split them). We can label a unit as "good" by typing "g", "mua" by typing "m" and "noise" by typing "n". We'll now all manually curate our analyzers. Here are some things to look out for:

### Curation: auto-correlograms

After spiking, a neuron has to rest for a while. So if a unit has two spikes within e.g. 1ms of each other, that's a sign that something in the sorting has gone wrong. The auto-correlogram helps you see these "refractory period violations". To make the histogram in the auto-correlogram plot, you ask: how many spikes occur between 0-1ms after another spike. Then: how many spikes occurs between 1-2ms after another spike? Then 2-3ms? And so on. Here's a low-event histogram.

![A simple auto-correlogram histogram. For this unit, ZERO spikes occured 0-1ms after another spike. Three spikes occured 1-2ms after another spike. Zero spikes occured 2-9ms after another spike, and one spike occured 9-10ms after another spike.](images/curate/curate_lilhist.png)

"Good" unit have no violations near 0ms. Many violations mean that the unit is contaminated with noise, or another unit. Here are a couple of examples:

![The auto-correlogram of a good unit](images/curate/curate_goodcorr.png)

![The auto-correlogram of a not-good unit](images/curate/curate_badcorr.png)
...
We see that the purple unit location on the probe matches where the peak of the signal is on the template plot. This unit looks "good" (to me) because 1) it's temporally localized 2) it has spatial decay 3) it has a trough, then a peak 4) it goes to zero on both sides. Note on the "main channel" view we can see that the unit's extrema value is -60uV.

Here's clear noise:

![The template of a "noise" unit](images/curate/curate_noisetemplate.png)

Here, the signal is the same on all electrodes (no spatial decay), it's temporally quite wide, it doesn't reach zero.

Finally, something more ambiguous:
...
Manual curation isn't fun. It also has genuine scientific issues: human decisions are flakey. If you do the same curation exercise tomorrow, you'll get a different result. If you're hungry, you'll probably be harsher on your units. If you've not seen a good unit for a while, you'll be desperate to label one as "good"! 

So, we can try to automate the process. We'll try three different methods, all based on _metrics_.
...
In our context a metric is a number which measures a property of the spike sorted unit. Let's see some common examples (the text here is adapted from the [UnitRefine paper](https://www.biorxiv.org/content/10.1101/2025.03.30.645770v2) by Jain et. al.)
  ...
  - Firing rate -- Average number of spikes in the cluster, per second.
  - Signal to noise ratio (snr) -- the extremal amplitude of the template on the extremal channel, divided by the standard deviation of the noise on this channel.
...
And some less common, but potentially just-as-important, ones:
  ...
  - drift ptp --The difference between the maximum and minimum values in the spike location difference distribution.
  - sync\_n -- Take all spike times from cluster $I$. Count all spikes which occur at the same timepoint (sample) as at least $n-1$ spikes from all other clusters. sync\_n is equal to this count.
  - exp decay -- 

Read more about the metrics either at the [UnitRefine paper](https://www.biorxiv.org/content/10.1101/2025.03.30.645770v2), or in the [SpikeInterface docs](https://spikeinterface.readthedocs.io/en/stable/modules/metrics.html). As you can see, they all measure properties of different stuff: the size of the signal, non-biological spike times, properties of the amplitude and depth distributions etc etc. There are around 40 metrics in SpikeInterface and each has its own intricacies.

In SpikeInterface, the metrics are contained in extensions. At the moment we separate them into two groups: quality metrics and template metrics. Hence you can compute them in a similar way you computed everything else:
...
The simplest approach is to _threshold_. This means: define some boundaries and only take units whose metrics live in those boundaries. Here's an example:
...
qm_labels = si.threshold_metrics_label_units(all_metrics, thresholds=qm_thresholds, column_name="simple_threshold")
...
```{python}
sorting_analyzer.set_sorting_property(key='simple_threshold', values=qm_labels['simple_threshold'].values.astype(str))
run_mainwindow(
    sorting_analyzer
)
```
...
I think of Bombcell, developed here in UCL by Julie Fabre, as _extreme_ thresholding. Roughly your unit goes through a decision tree, and depending on the metrics, it will get a different label. The decision tree is carefully thought out and takes into account all sorts of things. Take a look at the decision tree on the [Bombcell GitHub page](https://github.com/Julie-Fabre/bombcell).
...
There are also plotting tools so that you can see _why_ Bombcell labelled your units as it did. E.g. here is an "upset plot":

```{python}
si.plot_bombcell_labels_upset(sorting_analyzer, unit_labels=bombcell_labels['bombcell_label'])
```

This tells us which metrics made units fail. After looking at your results, you might decide to change the default parameters. To do you, you can first check out the default params:
...
Then decide which you change, and feed these into your labelling function:
...
In UnitRefine, we start with a small (say, 1000 units) manually labelled dataset. We then use machine learning to train a classifier based on this labelled data and the metrics of each unit. So these classifiers mimic the people who originally did the labelling. The hardest problem isn't training the classifiers - it's getting good labelled data. Studies show that humans agree approximately 80% of the time (), so this gives a ceiling on how well a classifier can mimic a group of manual curators.
...
sorting_analyzer.set_sorting_property(key='unitrefine_labels', values=unitrefine_labels['unitrefine_labels'].values.astype(str))
run_mainwindow(
    sorting_analyzer
)
```
...
We've now tried three labelling methods. And we've been storing their results in the analyzer. So we can compare them in the GUI. Here's a unit where all the methods disagree:
...
(As these cli commands get more complicated, you might see why you want to run SpikeInterface-GUI from a Python script rather than from terminal!)
...
I find that SLAy usually gives results I trust, although perhaps it misses some units. Everything you see in the GUI, you can recreate in SpikeInterface using the `compute_merge_unit_groups` function:
...
Finally, SpikeInterface-GUI allows you to split units which have been over-merged. The current generation of spike sorters are usually tuned to over-split units because manual merging is relatively easy. But manual splitting is difficult. Also, spikes which look "out of distribution" in some way are sometimes there for another reason: these events might be when the spikes overlap with spikes from another unit. Or they could be slightly deformed spikes taking place during sharp wave ripples. Overall: I don't recommend splitting.
...
You've curated your data. Congrats!!! After lunch we'll do some downstream analysis. For downstream analysis, you might want to use curated results. To do this, we'll load the original analyzer, load the curation, and apply the curation to the analyzer:
...
If you were being strict, you might also delete "mua" units.
