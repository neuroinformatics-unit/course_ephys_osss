...
Because we want to load the AP ("Action Potential") data stream for spike sorting, we pass `"imec0.ap"`.
...
By default, [Kilosort 4](https://github.com/MouseLand/Kilosort) will perform similar preprocessing steps to which we have already applied (it does highpass filtering and common average referencing).
...
The sorting output is written to disk. The output contains all of the output files of the sorter, and its organisation is sorter-dependent. As well as spike times and unit assignments, it contains other outputs from the sorter e.g. representative waveforms for each unit ('templates').
...
```{python}
waveforms = analyzer.get_extension("waveforms").get_data()
...
Finally, spike amplitudes are a useful measure for assessing unit quality. We expect that the amplitude of a spike is relatively stable throughout a recording. Changes in spike amplitudes may indicate problems with the neuron health, drift, or other issues. 
