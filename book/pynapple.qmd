...
We store a time series with corresponding data values, for example a speed curve, in a `Tsd` object.
...
This could be a collection of spiking units, for example.
```{python}
import pynapple as nap
tsgroup = nap.TsGroup({
    1: nap.Ts([1,2,3,4,5]), 
...
If we have multiple time series with data with the same time points, they can be stored in a `TsdFrame`.  
This could be a collection of calcium traces, for example.
```{python}
import pynapple as nap
...
We store time intervals as start and end pairs in an `IntervalSet`.
```{python}
import pynapple as nap
...
## 2. Interactions
Many Pynapple objects interact with each other.
...
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
fig.legend(frameon=False, bbox_to_anchor=(0.9, 1.1))
plt.show()
```
...
## 4. Core functions
Pynapple has a core set of functions designed to help with neuroscientific analyses.
...
### Averaging
The values of any time series with data can be averaged across bins.
```{python}
import pynapple as nap
import numpy as np
...
### Smoothing
Time series with data can be (Gaussian) smoothed.
```{python}
import pynapple as nap
import numpy as np
times = np.arange(0, 10, 0.01)
...
```
### Crosscorrelograms
Given one or two groups of time series, you can compute crosscorrelograms.
```{python}
...
Let's simulate a noisy multi-frequency signal:
```{python}
import pynapple as nap
import numpy as np
fs = 1000
...
#### Bandpass filter
We can bandpass filter to exract certain frequency bands.
```{python}
...
We can compute the power-spectral density to find frequencies of interest.
```{python}
...
### Saving as `.npz`
Pynapple objects can be saved (and loaded) as `.npz` files.
