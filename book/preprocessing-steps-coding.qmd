...
The working ranges are associated with the specific recording requiptment, most commonly the amplifier and analog-digital converters. For Neuropixels, these electronics are contained in the probe and the probe manual shows hte dynaimc range (1200 uV for NP1, XXX uV for NP2).

The data we are working with was recorded on a Camridge Neurotech probe, using a Intan RHD2000-series amplifier/headstage with range -5mV to 5mV. Therefore, we can use SpikeInterface to detect any periods of the recording that exceed 5mV (5000 uV).
