...
The goal of today is to take a look at our spike sorting results (using SpikeInterface-GUI). Once we look at them, we'll realise that all spike sorters make mistakes. To try and rememdy some of these mistakes, we'll try and curate the output. Before getting started, let's do a very quick review of what we've done so far.
...
TODO: Make consistent with Glossary
...
Note that some people call units "clusters". 

The common "mistakes" that spike sorters make are:
...
"Curation mode" allows us to label units (and merge and split them). We can label a unit as "good" by typing "g", "mua" by typing "m" and "noise" by typing "n". We'll now all manually curate our analyzers. Next we will look at a number of features of the sorted units that can help us determine whether they are "good", "mua" or "noise":

### Curation: auto-correlograms

After spiking, a neuron has to [rest for a while](https://en.wikipedia.org/wiki/Refractory_period_(physiology)). So if a unit has two spikes within e.g. 1ms of each other, that's a sign that something in the sorting has gone wrong as it is not physiologically possible for action potentials to fire that rapidly. 
...
TODO: write something about spike distributions

TODO: Make exercises all formatted in the same way.
...
In SpikeInterface, the metrics are contained in extensions. At the moment we separate them into two groups: quality metrics and template metrics. Hence you can compute them in a similar way you computed other `SortingAnalyzer` extensions:
...
In UnitRefine ([preprint](https://www.biorxiv.org/content/10.1101/2025.03.30.645770v2) + [gitub repo](https://github.com/anoushkajain/UnitRefine.git)), you start with a small (say, 1000 units) manually labelled dataset. UnitRefine then use machine learning to train a classifier based on this labelled data and the metrics of each unit. So these classifiers mimic the people who originally did the labelling. 

The hardest problem isn't training the classifiers - it's getting good labelled data. Studies show that humans agree approximately 80% of the time (TODO: add reference), so this gives a ceiling on how well a classifier can mimic a group of manual curators.
...
Finally, SpikeInterface-GUI allows you to split units which have been over-merged. Splitting means taking a unit consisting of n spikes, and splitting the spikes into two groups. The two seperate groups are then each assigned to a new unit. We've split one unit into two units.

The current generation of spike sorters are usually tuned to over-split units because manual merging is relatively easy. But manual splitting is difficult. Also, spikes which look like outliers in some way sometimes look like outliers for other reasons: these events might be when the spikes overlap with spikes from another unit. Or they could be slightly deformed spikes taking place during sharp wave ripples. Overall: I don't recommend splitting.
