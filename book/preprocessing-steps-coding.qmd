...
The electronics that record the extracellular voltage have dynamic ranges in which they operate. If the voltage exceeds this threshold, the equipment is said to 'saturate' and the recorded values are uninformative.
...
The data we are working with was recorded on a Camridge Neurotech probe, using a Intan RHD2000-series amplifier/headstage with range -5mV to 5mV. Therefore, we can use SpikeInterface to detect any periods of the recording that exceed 5mV (5000 μV).

We need to detect saturation events before any preprocessing has occurred, as the preprocessing steps might remove them. However, we must remove these events by zeroing them out after filtering, as sharp transitions to zero can cause filter artifacts. Therefore, we will detect the saturation periods now but actually zero out the data [later](#zeroing-the-saturation-artifacts).
...
We can see that the detected saturation event indeed exceeds the 5mV range. The data recorded here is thus uninformative and we will zero it out later.
