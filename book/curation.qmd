...
```{python}
sigui path/to/full_analyzer_lupin_000939.zarr
```
...
```{python}
sigui --layout-preset unit-focus path/to/full_analyzer_lupin_000939.zarr
```
...
```{python}
sigui --layout-file path/to/my_custom_layout.json path/to/full_analyzer_lupin_000939.zarr
```
...
In SpikeInterface a **Curation** is a list of decisions, formalised into a [Curation class](https://spikeinterface.readthedocs.io/en/stable/modules/curation.html#curation-format) (which we won't worry much about). These decisions are labels, merges and splits. As we proceed, we'll create **Curation**s by using the GUI or by using tools inside SpikeInterface.
...
```{python}
sigui --curation path/to/full_analyzer_lupin_000939.zarr
```
...
We have computed three spike distributions: spike amplitudes, amplitude scalings and spike locations. These are features _per spike_. E.g. the spike locations is the (estimated) x,y location of each spike on the probe while the spike amplitude is the magnitude of the (preprocessed) signal at the spike time (on the channel the unit is "maximal" on). These distributions can warn us of potential problems with the sorting. Here is a spike amplitudes distribution. Spike times are on the x-axis while spike amplitude is on the y-axis. On the far right, we plot a binned histogram of spike amplitudes which gives us a overall summary of the data.

![A spike amplitudes distribution](images/curate/amp_dist_1.png)

There are two striking things about this distribution. First, the spikes drop out completely for long stretches of time. Neurons are sometimes "quieter" or "louder" for a while. But when it is so striking and sudden like in this example, it is usually due to a spike sorting error. Second, spike amplitude changes a lot over time. It goes from around -450 microns at `time=0s`(econds) to -550 microns at `time=4000s`. When the amplitude changes this much, it's hard for the spike sorter to correctly sort this unit. We should expect that it could have made a mistake, like over-splitting. When we're merging units later, we should look out for this unit! Why does the amplitude change like this? It could be that the probe is moving slightly: as time goes on the electrode which measure the neuron gets closer to the neuron and so measures a stronger signal.

Here's another example:

![A spike amplitudes distribution](images/curate/amp_dist_2.png)

Here, the unit is present during the full recording. That's a good sign. But the distribution seems to "split", starting near `t=6000s`. On the left it looks like a unimodal normal distribution (that's what we expect). But on the right it seems to be bimodal. This _might_ be a sign that this unit is really the signal from two neurons: the sorter has "overmerged" two units during the second half of the recording. The spikes with the large signal near `time=8000s` are also a bit suspicious.
...
![A unit which has been classified as "noise" by simple thresholding, "sua" ("good") by the UnitRefine model and "mua" by Bombcell.](images/curate/unit_with_different_labels.png)
...
```{python}
sigui --curation --layout-preset merge_focus path/to/full_analyzer_lupin_000939.zarr
```
